...
def _build_stack(max_size: int, value: int = -1) -> Array:
    return jnp.full((max_size,), value, dtype=INDEX_DTYPE)


def _add_child_if_valid(
    stack: Array,
    stack_size: int,
    overflow: bool,
    child: int,
    *,
    ...
    def when_valid(args):
        ...
        def push_ok(state):
            stk_inner, size_inner, of_inner = state
            stk_inner = stk_inner.at[size_inner].set(ch)
            return stk_inner, size_inner + 1, of_inner

        def push_over(state):
            stk_inner, size_inner, _of_inner = state
            return stk_inner, size_inner, jnp.bool_(True)
    ...
    return lax.cond(
        ...
        when_valid,
        when_invalid,
        (stack, stack_size, overflow, child),
    )
...
def _push_children(
    ...
    *,
    ...
    if left_child.shape[0] == 0:
        return stack, stack_size, overflow

    def when_internal(state):
        stk, size, of = state
        left = lax.dynamic_index_in_dim(
            left_child,
            node,
            axis=0,
            keepdims=False,
        )
        right = lax.dynamic_index_in_dim(
            right_child,
            node,
            axis=0,
            keepdims=False,
        )
        stk, size, of = _add_child_if_valid(
            stk,
            size,
            of,
            left,
    ...
    return lax.cond(
        (node >= 0) & (node < num_internal),
        when_internal,
        lambda state: state,
        (stack, stack_size, overflow),
    )
...
def _add_source(
    ...
    *,
    ...
    def when_ok(args):
        row, cnt, of, current = args
        row = row.at[cnt].set(current)
        return row, cnt + 1, of

    def when_full(args):
        row, cnt, _of, current = args
        return row, cnt, jnp.bool_(True)

    return lax.cond(
        count < max_interactions,
        when_ok,
        when_full,
        (sources_row, count, overflow, node),
    )
